...
def main():
    ...
    p.add_argument("--clip-guidance-scale", "-cs", type=float, default=1000.0, help="the CLIP guidance scale")
    p.add_argument("--eta", type=float, default=1.0, help="the amount of noise to add during sampling (0-1)")
    p.add_argument("--init", type=str, help="the init image")
    p.add_argument("--model", type=str, choices=get_models(), required=True, help="the model to use")
    ...
    clip_patch_size = 16
    clip_size = 224
    normalize = make_normalize(mean=[0.48145466, 0.4578275, 0.40821073], std=[0.26862954, 0.26130258, 0.27577711])
    ...
    def clip_cond_fn_loss(x, key, params, clip_params, t, extra_args):
        ...
        sat_vmap = jax.vmap(partial(jax.image.scale_and_translate, method="cubic"), in_axes=(0, None, None, 0, 0))
    ...
    def run(key, n):
        ...
        sample_step = partial(
            sampling.jit_cond_sample_step, extra_args={}, cond_fn=clip_cond_fn, cond_params=cond_params
        )
